...
function init() {
  ...
  root.camera.position.set(0, 0, 45);
  ...
  l2.load('assets/dist/img/slider2.jpg', function(img) {
    slide2.setImage(img);
  })
  ...
  l3.load('assets/dist/img/slider3.jpg', function(img) {
    slide3.setImage(img);
  })
...
function Slide(width, height, animationPhase) {
  ...
  var material = new THREE.BAS.BasicAnimationMaterial({
    ...
    shaderVertexInit: [
      'float tDelay = aAnimation.x;', 'float tDuration = aAnimation.y;', 'float tTime = clamp(uTime - tDelay, 0.0, tDuration);', 'float tProgress = ease(tTime, 0.0, 1.0, tDuration);'
...
THREERoot.prototype = {
  ...
  resize: function() {
    this.camera.aspect = 360 / 740;
    var tanFOV = Math.tan( ( ( Math.PI / 180 ) * this.camera.fov / 2 ) );
    this.camera.fov = ( 360 / Math.PI ) * Math.atan( tanFOV * ( 740 / 360 ) );


    this.camera.updateProjectionMatrix();
    this.camera.lookAt( this.scene.position );

    this.renderer.setSize(360, 740);
    this.renderer.render( this.scene, this.camera );
  }
};
